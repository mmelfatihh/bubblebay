...
export const Room = ({ users, onHangUp }: RoomProps) => {
  ...
  // 1. INITIALIZE PHYSICS (With explicit re-run)
  useEffect(() => {
    ...
    users.forEach(u => {
        // If this user exists but has no physics body yet, create it
  ...
  }, [users]); // This fires every time the user list updates
    ...
    <div className="absolute inset-0 z-40 pointer-events-none"> 
      ...
      <div className="absolute inset-0 pointer-events-auto"> 
        {users.map(user => {
            ...
            // Use local mic for ME, use nothing for others (for now)
                ...
                <div key={user.id}>
                    ...
                    {isVisible && (
                        <RoomBubble
                            ...
                            y={state.y}
                            isMe={user.isMe} 
                            participant={user.participant} // Pass the real participant object
                            volume={volume} 
